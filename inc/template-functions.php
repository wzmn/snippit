 ...
 * @return array
 */
function cogncise_body_classes( $classes ) {
...
add_filter( 'body_class', 'cogncise_body_classes' );
 ...
 * @return array
 */
function cogncise_post_classes( $classes ) {
	$classes[] = 'entry';

	return $classes;
}
add_filter( 'post_class', 'cogncise_post_classes', 10, 3 );
 ...
 * @return void
 */
function cogncise_pingback_header() {
	if ( is_singular() && pings_open() ) {
		echo '<link rel="pingback" href="', esc_url( get_bloginfo( 'pingback_url' ) ), '">';
	}
}
add_action( 'wp_head', 'cogncise_pingback_header' );
 ...
 * @return void
 */
function cogncise_supports_js() {
	echo '<script>document.body.classList.remove("no-js");</script>';
}
add_action( 'wp_footer', 'cogncise_supports_js' );
 ...
 * @return array
 */
function cogncise_comment_form_defaults( $defaults ) {
...
add_filter( 'comment_form_defaults', 'cogncise_comment_form_defaults' );
 ...
 * @return bool
 */
function cogncise_can_show_post_thumbnail() {
	/**
	...
	return apply_filters(
		'cogncise_can_show_post_thumbnail',
 ...
 * @return int
 */
function cogncise_get_avatar_size() {
	return 60;
}
 ...
 * @since Cogncise 1.0
 */
function cogncise_continue_reading_text() {
	$continue_reading = sprintf(
		/* translators: %s: Post title. Only visible to screen readers. */
		esc_html__( 'Continue reading %s', 'cogncise' ),
 ...
 * @since Cogncise 1.0
 */
function cogncise_continue_reading_link_excerpt() {
	if ( ! is_admin() ) {
		return '&hellip; <a class="more-link" href="' . esc_url( get_permalink() ) . '">' . cogncise_continue_reading_text() . '</a>';
	}
}

// Filter the excerpt more link.
add_filter( 'excerpt_more', 'cogncise_continue_reading_link_excerpt' );
 ...
 * @since Cogncise 1.0
 */
function cogncise_continue_reading_link() {
	if ( ! is_admin() ) {
		return '<div class="more-link-container"><a class="more-link" href="' . esc_url( get_permalink() ) . '#more-' . esc_attr( get_the_ID() ) . '">' . cogncise_continue_reading_text() . '</a></div>';
	}
}

// Filter the content more link.
add_filter( 'the_content_more_link', 'cogncise_continue_reading_link' );

if ( ! function_exists( 'cogncise_post_title' ) ) {
	/**
	 ...
	 * @return string
	 */
	function cogncise_post_title( $title ) {
		return '' === $title ? esc_html_x( 'Untitled', 'Added to posts and pages that are missing titles', 'cogncise' ) : $title;
	}
}
add_filter( 'the_title', 'cogncise_post_title' );
 ...
 * @return string
 */
function cogncise_get_icon_svg( $group, $icon, $size = 24 ) {
	return Cogncise_SVG_Icons::get_svg( $group, $icon, $size );
}
 ...
 * @return string
 */
function cogncise_change_calendar_nav_arrows( $calendar_output ) {
	$calendar_output = str_replace( '&laquo; ', is_rtl() ? cogncise_get_icon_svg( 'ui', 'arrow_right' ) : cogncise_get_icon_svg( 'ui', 'arrow_left' ), $calendar_output );
	$calendar_output = str_replace( ' &raquo;', is_rtl() ? cogncise_get_icon_svg( 'ui', 'arrow_left' ) : cogncise_get_icon_svg( 'ui', 'arrow_right' ), $calendar_output );
	return $calendar_output;
}
add_filter( 'get_calendar', 'cogncise_change_calendar_nav_arrows' );
 ...
 * @return string
 */
function cogncise_get_non_latin_css( $type = 'front-end' ) {
	...
	$font_family = apply_filters(
		'cogncise_get_localized_font_family_types',
		array(
	...
	$elements = apply_filters(
		'cogncise_get_localized_font_family_elements',
		array(
	...
	if ( ! function_exists( 'cogncise_generate_css' ) ) {
	...
	return cogncise_generate_css( // @phpstan-ignore-line.
 ...
 */
function cogncise_print_first_instance_of_block( $block_name, $content = null, $instances = 1 ) {
	$instances_count = 0;
	$blocks_content  = '';
 ...
 *
 * @since Cogncise 1.0
 * @since Cogncise 1.4 Corrected parameter name for `$output`,
 ...
 */
function cogncise_password_form( $output, $post = 0 ) {
	$post   = get_post( $post );
	$label  = 'pwbox-' . ( empty( $post->ID ) ? wp_rand() : $post->ID );
	$output = '<p class="post-password-message">' . esc_html__( 'This content is password protected. Please enter a password to view.', 'cogncise' ) . '</p>
	<form action="' . esc_url( site_url( 'wp-login.php?action=postpass', 'login_post' ) ) . '" class="post-password-form" method="post">
	<label class="post-password-form__label" for="' . esc_attr( $label ) . '">' . esc_html_x( 'Password', 'Post password form', 'cogncise' ) . '</label><input class="post-password-form__input" name="post_password" id="' . esc_attr( $label ) . '" type="password" size="20" /><input type="submit" class="post-password-form__submit" name="' . esc_attr_x( 'Submit', 'Post password form', 'cogncise' ) . '" value="' . esc_attr_x( 'Enter', 'Post password form', 'cogncise' ) . '" /></form>
	';
	return $output;
}
add_filter( 'the_password_form', 'cogncise_password_form', 10, 2 );
 ...
 */
function cogncise_get_attachment_image_attributes( $attr, $attachment, $size ) {
...
add_filter( 'wp_get_attachment_image_attributes', 'cogncise_get_attachment_image_attributes', 10, 3 );
